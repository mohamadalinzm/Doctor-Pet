...
class StoreTest extends TestCase
{
    public function test_store_validation()
    {
        ...
        $mockValidationService = $this->getMockBuilder(AnimalValidationService::class)
                                      ->onlyMethods(['store'])
                                      ->getMock();
        ...
        $mockValidationService->expects($this->once())
                              ->method('store')
                              ->with(['Animal' => '123 Main St', 'city' => 'New York', 'state' => 'NY', 'zip' => '10001']);

        // Call the store method of the validation service
        $mockValidationService->store(['Animal' => '123 Main St', 'city' => 'New York', 'state' => 'NY', 'zip' => '10001']);
    }
    ...
    public function test_store_action()
    {
        ...
        $mockActionService->store(['Animal' => '123 Main St', 'city' => 'New York', 'state' => 'NY', 'zip' => '10001']);
    }
    ...
    public function test_store_event()
    {
        ...
        // Set up the mock to expect the create method to be called with the authenticated user and the Animal object
        $mockEventService->expects($this->once())
                         ->method('create')
                         ->with($this->equalTo(auth()->user()), $this->isInstanceOf(Animal::class));

        // Call the create method of the event service
        $mockEventService->create(auth()->user(), Animal::factory()->create());
    }
    ...
    public function test_store_response()
    {
        ...
        $mockResponseService = $this->getMockBuilder(ResponseService::class)
                                    ->onlyMethods(['AnimalStored'])
                                    ->getMock();

        // Set up the mock to expect the AnimalStored method to be called
        $mockResponseService->expects($this->once())
                            ->method('AnimalStored');

        // Call the AnimalStored method of the response service
        $mockResponseService->AnimalStored();
    }
